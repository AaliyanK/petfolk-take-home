...
- Diagram, three tries: (1) a mermaid block, which showed as raw text in Aaliyan's markdown previewer; (2) a hand-authored SVG (`architecture.svg`), which rendered clean but Aaliyan found busy; (3) the final form, a plain-text box-drawing flow inside a fenced code block, in the vertical `│ ▼ ├── └──►` style Aaliyan asked for. Renders identically everywhere, no dependency. SVG removed.
