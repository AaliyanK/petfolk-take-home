...
- Built `question-2/build.md`: current state (quoted Pengo3 messages), the architecture, method-to-machine table, the review model (templates as the unit, verify_quotes.py as a runtime gate, fail closed, sampled audit, kill switch, a clinical-comms owner role that must be created), the reply path, rollout (shadow -> discharge-summary enrichment -> expand), the retiered budget, gaps/risks, AI/human split. ~1300 words.
- Diagram: tried a mermaid block first (Aaliyan's markdown previewer showed it as raw text). Tried a hand-authored SVG (`question-2/architecture.svg`, rendered clean, palette-matched) but Aaliyan found it messy. Settled on **no diagram**: the architecture is a well-formatted numbered flow in the markdown itself (event -> classify -> select -> guardrail -> send or fail closed, plus the webhook and template-approval loop). SVG file removed.
